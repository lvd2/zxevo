...
void cpu_info()
{
   ...
   printf("%d.%d.%d [MMX:%s,SSE:%s,SSE2:%s,TSCINV:%s] ",
      ...
      temp.sse ? "YES" : "NO",
      temp.sse2 ? "YES" : "NO",
      features_apm_supported ? ((features_apm & (1 << 8)) ? "YES" : "NO") : "UNK");
...
//   printf("%s\n", __FUNCTION__);
   ...
   temp.PngSupport = PngInit();
	if(conf.wiznet){
		Wiz5300::Init();
		int i=(Wiz5300::RegRead(0xfe)<<8)|Wiz5300::RegRead(0xff);
   ...
   if(!temp.PngSupport)
   {
       ...
       if(conf.scrshot == 2) // Был выбран тип скриншотов .png
       {
           conf.scrshot = 1; // Замена на .bmp (т.к. отсутствует libpng)
       }
   }
