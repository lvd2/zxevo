...
//#ifndef Z80_COMMON
static Z80OPCODE ope_40(Z80 *cpu) { // in b,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->b = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->b] | (cpu->f & CF);
}
static Z80OPCODE ope_41(Z80 *cpu) { // out (c),b | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_42(Z80 *cpu) { // sbc hl,bc
   cpu->memptr = cpu->hl+1;
   unsigned char fl = NF;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_43(Z80 *cpu) { // ld (nnnn),bc | M:6 T:20 (4, 4, 3, 3, 3, 3)
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_44(Z80 *cpu) { // neg
   cpu->f = sbcf[cpu->a];
   cpu->a = -cpu->a;
}
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_45(Z80 *cpu) { // retn
   cpu->iff1 = cpu->iff2;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_46(Z80 *cpu) { // im 0
   cpu->im = 0;
}
static Z80OPCODE ope_47(Z80 *cpu) { // ld i,a
   cpu->i = cpu->a;
   cpu->t++;
}
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_48(Z80 *cpu) { // in c,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->c = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->c] | (cpu->f & CF);
}
static Z80OPCODE ope_49(Z80 *cpu) { // out (c),c | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_4A(Z80 *cpu) { // adc hl,bc
   cpu->memptr = cpu->hl+1;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_4B(Z80 *cpu) { // ld bc,(nnnn)
   unsigned adr = cpu->MemIf->xm(cpu->pc++);
   adr += cpu->MemIf->xm(cpu->pc++)*0x100;
   cpu->memptr = adr+1;
...
static Z80OPCODE ope_4D(Z80 *cpu) { // reti
   cpu->iff1 = cpu->iff2;
   unsigned addr = cpu->MemIf->rm(cpu->sp++);
   addr += 0x100*cpu->MemIf->rm(cpu->sp++);
   cpu->last_branch = u16(cpu->pc-2);
...
//#ifdef Z80_COMMON
static Z80OPCODE ope_4F(Z80 *cpu) { // ld r,a
   cpu->r_low = cpu->a;
   cpu->r_hi = cpu->a & 0x80;
   cpu->t++;
}
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_50(Z80 *cpu) { // in d,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->d = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->d] | (cpu->f & CF);
}
static Z80OPCODE ope_51(Z80 *cpu) { // out (c),d | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_52(Z80 *cpu) { // sbc hl,de
   cpu->memptr = cpu->hl+1;
   unsigned char fl = NF;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_53(Z80 *cpu) { // ld (nnnn),de | M:6 T:20 (4, 4, 3, 3, 3, 3)
   unsigned adr = cpu->MemIf->xm(cpu->pc++);
   adr += cpu->MemIf->xm(cpu->pc++)*0x100;
   cpu->memptr = adr+1;
...
//#ifdef Z80_COMMON
static Z80OPCODE ope_56(Z80 *cpu) { // im 1
   cpu->im = 1;
}

static Z80OPCODE ope_57(Z80 *cpu)
{ // ld a,i
   cpu->a = cpu->i;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_58(Z80 *cpu) { // in e,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->e = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->e] | (cpu->f & CF);
}
static Z80OPCODE ope_59(Z80 *cpu) { // out (c),e | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_5A(Z80 *cpu) { // adc hl,de
   cpu->memptr = cpu->hl+1;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_5B(Z80 *cpu) { // ld de,(nnnn)
   unsigned adr = cpu->MemIf->xm(cpu->pc++);
   adr += cpu->MemIf->xm(cpu->pc++)*0x100;
   cpu->memptr = adr+1;
...
//#ifdef Z80_COMMON
static Z80OPCODE ope_5E(Z80 *cpu) { // im 2
   cpu->im = 2;
}

static Z80OPCODE ope_5F(Z80 *cpu)
{ // ld a,r
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_60(Z80 *cpu) { // in h,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->h = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->h] | (cpu->f & CF);
}
static Z80OPCODE ope_61(Z80 *cpu) { // out (c),h | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_62(Z80 *cpu) { // sbc hl,hl
   cpu->memptr = cpu->hl+1;
   unsigned char fl = NF;
...
//#ifndef Z80_COMMON
static Z80OPCODE ope_67(Z80 *cpu) { // rrd | M:5 T:18 (4, 4, 3, 4, 3)
   unsigned char tmp = cpu->MemIf->rm(cpu->hl);
   cpu->memptr = cpu->hl+1;
   cpu->MemIf->wm(cpu->hl, u8(cpu->a << 4) | u8(tmp >> 4));
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_68(Z80 *cpu) { // in l,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->l = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->l] | (cpu->f & CF);
}
static Z80OPCODE ope_69(Z80 *cpu) { // out (c),l | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_6A(Z80 *cpu) { // adc hl,hl
   cpu->memptr = cpu->hl+1;
...
//#ifndef Z80_COMMON
static Z80OPCODE ope_6F(Z80 *cpu) { // rld | M:5 T:18 (4, 4, 3, 4, 3)
   unsigned char tmp = cpu->MemIf->rm(cpu->hl);
   cpu->memptr = cpu->hl+1;
   cpu->MemIf->wm(cpu->hl, u8(cpu->a & 0x0F) | u8(tmp << 4));
   cpu->a = (cpu->a & 0xF0) | (tmp >> 4);
   cpu->f = log_f[cpu->a] | (cpu->f & CF);
   cpu->t += 10;
}
static Z80OPCODE ope_70(Z80 *cpu) { // in (c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->f = log_f[cpu->in(cpu->bc)] | (cpu->f & CF);
}
static Z80OPCODE ope_71(Z80 *cpu) { // out (c),0
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->out(cpu->bc, 0);
}
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_72(Z80 *cpu) { // sbc hl,sp
   cpu->memptr = cpu->hl+1;
   unsigned char fl = NF;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_73(Z80 *cpu) { // ld (nnnn),sp | M:6 T:20 (4, 4, 3, 3, 3, 3)
   unsigned adr = cpu->MemIf->xm(cpu->pc++);
   adr += cpu->MemIf->xm(cpu->pc++)*0x100;
   cpu->memptr = adr+1;
...
//#ifdef Z80_COMMON
static Z80OPCODE ope_76(Z80 *cpu) { // im 1
   cpu->im = 1;
}
//#endif
...
//#ifndef Z80_COMMON
static Z80OPCODE ope_78(Z80 *cpu) { // in a,(c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->a = cpu->in(cpu->bc);
   cpu->f = log_f[cpu->a] | (cpu->f & CF);
}
static Z80OPCODE ope_79(Z80 *cpu) { // out (c),a | M:3 T:12 (4, 4, 4)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
...
//#endif
//#ifdef Z80_COMMON
static Z80OPCODE ope_7A(Z80 *cpu) { // adc hl,sp
   cpu->memptr = cpu->hl+1;
...
//#endif
//#ifndef Z80_COMMON
static Z80OPCODE ope_7B(Z80 *cpu) { // ld sp,(nnnn)
   unsigned adr = cpu->MemIf->xm(cpu->pc++);
   adr += cpu->MemIf->xm(cpu->pc++)*0x100;
   cpu->memptr = adr+1;
...
//#ifndef Z80_COMMON
static Z80OPCODE ope_A0(Z80 *cpu) { // ldi | M:4 T:16 (4, 4, 3, 5)
   cpu->t += 8;
...
static Z80OPCODE ope_A1(Z80 *cpu) { // cpi
   cpu->t += 8;
...
static Z80OPCODE ope_A2(Z80 *cpu) { // ini | M:4 T:16 (4, 5, 3, 4)
   cpu->memptr = cpu->bc+1;
   cpu->t += 8;
...
static Z80OPCODE ope_A3(Z80 *cpu) { // outi | M:4 T:16 (4, 5, 3, 4)
   cpu->t += 8;
   cpu->b--;
...
static Z80OPCODE ope_A8(Z80 *cpu) { // ldd | M:4 T:16 (4, 4, 3, 5)
   cpu->t += 8;
   unsigned char tempbyte = cpu->MemIf->rm(cpu->hl--);
   cpu->MemIf->wm(cpu->de--, tempbyte);
   tempbyte += cpu->a; tempbyte = (tempbyte & F3) + ((tempbyte << 4) & F5);
...
static Z80OPCODE ope_A9(Z80 *cpu) { // cpd
   cpu->t += 8;
   unsigned char cf = cpu->f & CF;
   unsigned char tempbyte = cpu->MemIf->rm(cpu->hl--);
   cpu->f = cpf8b[cpu->a*0x100 + tempbyte] + cf;
...
static Z80OPCODE ope_AA(Z80 *cpu) { // ind | M:4 T:16 (4, 5, 3, 4)
   cpu->memptr = cpu->bc-1;
   cpu->t += 8;
   u8 tmp = cpu->in(cpu->bc);
   cpu->MemIf->wm(cpu->hl--, tmp);
...
static Z80OPCODE ope_AB(Z80 *cpu) { // outd | M:4 T:16 (4, 5, 3, 4)
   cpu->t += 8;
   cpu->b--;
...
static Z80OPCODE ope_B0(Z80 *cpu) { // ldir | BC!=0 M:5 T:21 (4, 4, 3, 5, 5) | BC==0 M:4 T:16 (4, 4, 3, 5)
    cpu->t += 8;
    unsigned char tempbyte = cpu->MemIf->rm(cpu->hl++);
    cpu->MemIf->wm(cpu->de++, tempbyte);
    tempbyte += cpu->a; tempbyte = (tempbyte & F3) + ((tempbyte << 4) & F5);
...
static Z80OPCODE ope_B1(Z80 *cpu) { // cpir
    cpu->memptr++;
    cpu->t += 8;
    unsigned char cf = cpu->f & CF;
    unsigned char tempbyte = cpu->MemIf->rm(cpu->hl++);
...
static Z80OPCODE ope_B2(Z80 *cpu) { // inir | BC!=0 M:5 T:21 (4, 5, 3, 4, 5) | BC==0 M:4 T:16 (4, 5, 3, 4)
    cpu->t += 8;
    cpu->memptr = cpu->bc + 1;
...
static Z80OPCODE ope_B3(Z80 *cpu) { // otir | B!=0 M:5 T:21 (4, 5, 3, 4, 5) | B==0 M:4 T:16 (4, 5, 3, 4)
    cpu->t += 8;
    dec8(cpu, cpu->b);
...
static Z80OPCODE ope_B8(Z80 *cpu) { // lddr | BC!=0 M:5 T:21 (4, 4, 3, 5, 5) | BC==0 M:4 T:16 (4, 4, 3, 5)
    cpu->t += 8;
    unsigned char tempbyte = cpu->MemIf->rm(cpu->hl--);
    cpu->MemIf->wm(cpu->de--, tempbyte);
    tempbyte += cpu->a; tempbyte = (tempbyte & F3) | ((tempbyte << 4) & F5);
...
static Z80OPCODE ope_B9(Z80 *cpu) { // cpdr
    cpu->memptr--;
    cpu->t += 8;
    unsigned char cf = cpu->f & CF;
    unsigned char tempbyte = cpu->MemIf->rm(cpu->hl--);
...
static Z80OPCODE ope_BA(Z80 *cpu) { // indr | BC!=0 M:5 T:21 (4, 5, 3, 4, 5) | BC==0 M:4 T:16 (4, 5, 3, 4)
    cpu->t += 8;
    cpu->memptr = cpu->bc - 1;
    cpu->MemIf->wm(cpu->hl--, cpu->in(cpu->bc));
    dec8(cpu, cpu->b);
...
static Z80OPCODE ope_BB(Z80 *cpu) { // otdr | B!=0 M:5 T:21 (4, 5, 3, 4, 5) | B==0 M:4 T:16 (4, 5, 3, 4)
   cpu->t += 8;
   dec8(cpu, cpu->b);
   cpu->out(cpu->bc, cpu->MemIf->rm(cpu->hl--));
   if(cpu->b)
