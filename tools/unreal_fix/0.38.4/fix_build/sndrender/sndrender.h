...
class SNDRENDER
{
 ...
 protected:
   ...
   unsigned clock_rate; // Входная частота (на этой частоте клиент делает запись в SNDRENDER)
   unsigned sample_rate; // Выходная частота (на этой частоте формируется выходной pcm)

 private:

   unsigned base_tick;  // Текущий такт с момента инициализации на частоте оверсэмплинга
   unsigned tick;// Такт от начала кадра на частоте оверсэмплинга
   unsigned s1_l, s1_r;
   unsigned s2_l, s2_r;
