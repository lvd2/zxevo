...
static int readPAL()
{
    ...
    if(conf.ula_plus)
    {
        comp.ula_plus_en = true; // Если присутствует ULA+, то сразу и активируем ее (чтобы изменения палитры были видны)
    }
    return 1;
}
...
static void SaveBmp16c(FILE *File, const u8 *ds)
{
    ...
    *(unsigned*)(bmpheader32 + 2) = 320 * 200 / 2 + sizeof(bmpheader32); // filesize
    ...
    {
        // Формат данных палитры Gg0Rr0Bb
        // Раскладка палитры эмулятора - ULA+
