...
{
    // ћаска кода размера сектора (3 - дл€ fd1793, 0xF - дл€ uPD765)
...
void TRKCACHE::seek(FDD *d, unsigned cyl, unsigned side, SEEK_MODE fs)
{
   ...
   if(cyl >= d->cyls) // —лучай увеличени€ размера диска при форматировании дополнительных дорожек до MAX_CYLS
   {
       if(fs != JUST_SEEK)
       {
           trkd = nullptr;
   ...
   {
      if (trkd[i] != 0xA1 || trkd[i+1] != 0xFE || !test_i(i)) // ѕоиск idam
          continue;

      if (s == MAX_SEC)
          errexit("too many sectors");

      SECHDR *h = &hdr[s++]; // «аполнение заголовка
      h->id = trkd + i + 2; // ”казатель на заголовок сектора
      h->c = h->id[0];
      h->s = h->id[1];
      h->n = h->id[2];
      h->l = h->id[3];
...
//      if (h->l > 5) continue; [vv]
      ...
      // ‘ормирование указател€ на зону данных сектора
      for (unsigned j = i + 8; j < end; j++)
      {
         if (trkd[j] != 0xA1 || !test_i(j) || test_i(j+1))
             continue;

         if (trkd[j+1] == 0xF8 || trkd[j+1] == 0xFB) // Ќайден data am (0xFB), deleted data am (0xF8)
         {
            ...
            h->data = trkd + j + 2;
            // ƒл€ больших секторов 8192 байта (не влезающих на дорожку)
            // до проверки crc дело не доходит, команда прерываетс€ по второму прохождению индекса
            ...
            if(trkwp)
            {
                for(unsigned b = 0; b < h->datlen; b++)
                {
                    if(test_wp(j + 2 + b))
                    {
                        h->wp_start = j + 2; // ≈сть хот€бы один сбойный байт
                        break;
                    }
                }
            }
...
{
   if(!trkd) // “рэк без данных (форматирование не нужно)
   {
       return;
   }
   ...
   { // ѕревышение стандартной длины дорожки, сокращаем параметры до минимальных
       gap4a = 1;
       sync0 = 1;
       i_am = 1;
       gap1 = 1;
...
//         if (sechdr->l > 5) errexit("strange sector"); // [vv]
         ...
         if (sechdr->data != (unsigned char*)1)
         {
             ...
             if(sechdr->wp) //  опирование битовой карты сбойных байтов
             {
...
//   dump();
   ...
   // возможно надо выставл€ть флаг не совпадени€ цилиндра, либо номер цилиндра вообще не вли€ет на поиск сектора
   // т.е. вли€ет только номер сектора и код его размера
